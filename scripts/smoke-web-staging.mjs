...
const save=await api('/v1/save',null,login.token);assert.equal(save.hintCount,1);
...
const reward=await api('/v1/ads/rewarded/claim',claim,login.token);assert.equal(reward.save.hintCount,save.hintCount+1);
const retry=await api('/v1/ads/rewarded/claim',claim,login.token);assert.equal(retry.save.hintCount,save.hintCount+1);
assert.equal((await api('/v1/save',null,login.token)).hintCount,save.hintCount+1);
