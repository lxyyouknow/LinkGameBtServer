...
[ "${BT_RUNTIME:-}" = 'web-preview' ] || { echo 'BT 远端 BT_RUNTIME 与发布清单不一致' >&2; exit 2; }
...
[ "${TIKTOK_CLIENT_KEY:-}" = '' ] || { echo 'BT 远端 TIKTOK_CLIENT_KEY 与发布清单不一致' >&2; exit 2; }
[ "${ENABLE_TIKTOK_LOGIN:-}" = 'false' ] || { echo 'BT 远端 ENABLE_TIKTOK_LOGIN 与发布清单不一致' >&2; exit 2; }
